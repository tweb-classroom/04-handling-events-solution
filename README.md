...
You are asked to improve the `Game` class and to implement and test the `keyboard` function. 
Notice, that some design decisions depend on the multiplayer features that will be implemented in the future assignments.
...
Hence, the `set(id, entity)`, `get(id)` and `delete(id)` methods provided by the `Map` class can be used to manage the moving entities during their lifecycle. 
You are asked to implement this class according to the tests provided in test/state.js. 
...
You are asked to implement this class and to provide a unit tests in the `test/keyboard.js` file.
...
For the corrections, we will check that you project passes the provided tests, read the tests your created and visually verify in the browser that the behaves as in the demonstration video.
